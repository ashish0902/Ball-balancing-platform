...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
	...
	##############	ADD YOUR CODE HERE	##############
	arr = np.array(vision_sensor_image,np.uint8)
	arr = arr.reshape((1024,1024,3))
	# print(arr.shape)
	transformed_image = cv2.cvtColor(arr,cv2.COLOR_BGR2GRAY)
	transformed_image = cv2.flip(transformed_image, 0)
	transformed_image = cv2.resize(transformed_image,(1280,1280))
